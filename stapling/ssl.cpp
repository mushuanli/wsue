...
#define FORMAT_HTTPCHECK_REQ(buf,serv,port) snprintf(buf,sizeof(buf)-1,"GET / HTTP/1.1\r\nHost: %s:%d\r\n\r\n",serv,port)
...
open_secure_connection(session *ssn,const char* serv,const char* sslproto)
{
	...
	if (sslctx)
		ctx = sslctx;
#if 0 // ignore ssl protocol
	if (sslctx)
		ctx = sslctx;

	if (sslproto) {
#ifndef OPENSSL_NO_SSL3_METHOD
		if (sslctx && !strcasecmp(sslproto, "ssl3"))
			ctx = sslctx;
#endif
#ifndef OPENSSL_NO_TLS1_METHOD
		if (sslctx && !strcasecmp(sslproto, "tls1"))
			ctx = sslctx;
#endif
#ifndef OPENSSL_NO_TLS1_1_METHOD
		if (sslctx && !strcasecmp(sslproto, "tls1.1"))
			ctx = sslctx;
#endif
#ifndef OPENSSL_NO_TLS1_2_METHOD
		if (sslctx && !strcasecmp(sslproto, "tls1.2"))
			ctx = sslctx;
#endif
	}
#endif
	...
	if (!(ssn->sslconn = SSL_new(ctx)))
        ...
        ocsp_initssl(ssn->sslconn,sslctx);
...
#if 0 // OPENSSL_VERSION_NUMBER >= 0x1010000fL
...
int VMISSL_CheckAlive(const char* serv,int port)
{
    ...
    error("read %d %d:%s:%s from %s:%d\n",status,ret,readbuf,p,serv,port);
    return status;
}
